...
<head>
    ...
    <title>{{ $title ?? 'Partner Dashboard' }} - {{ config('app.name', 'Portal Catering') }}</title>
    <!-- Favicon -->
            ...
            <li class="nav-item dropdown user-menu">
                ...
                <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                    ...
                    <li class="user-header bg-primary">
                        ...
                        <p>
                            {{ auth()->user()->name }}
                            <small>{{ auth()->user()->cateringPartner->business_name }}</small>
                        </p>
                    </li>
                    <!-- Menu Footer-->
                    <li class="user-footer">
                        <a href="{{ route('partner.profile.edit') }}" class="btn btn-default btn-flat">Profil</a>
    ...
    <aside class="main-sidebar sidebar-dark-primary elevation-4">
        ...
        <a href="{{ route('partner.dashboard') }}" class="brand-link">
            @if(auth()->user()->cateringPartner->logo)
                <img src="{{ asset('storage/' . auth()->user()->cateringPartner->logo) }}" alt="{{ auth()->user()->cateringPartner->business_name }}" class="brand-image img-circle elevation-3" style="opacity: .8">
            @else
                <img src="{{ asset('vendor/adminlte/dist/img/AdminLTELogo.png') }}" alt="{{ auth()->user()->cateringPartner->business_name }}" class="brand-image img-circle elevation-3" style="opacity: .8">
            @endif
        ...
        <div class="sidebar">
            ...
            <nav class="mt-2">
                <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                    ...
                    <li class="nav-item">
                        <a href="{{ route('partner.menus.index') }}" class="nav-link {{ request()->routeIs('partner.menus.*') ? 'active' : '' }}">
                            <i class="nav-icon fas fa-utensils"></i>
                            <p>Menu Catering</p>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('partner.orders.index') }}" class="nav-link {{ request()->routeIs('partner.orders.*') ? 'active' : '' }}">
                            <i class="nav-icon fas fa-shopping-cart"></i>
                            <p>Pesanan</p>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('partner.reports.monthly') }}" class="nav-link {{ request()->routeIs('partner.reports.*') ? 'active' : '' }}">
                            <i class="nav-icon fas fa-chart-bar"></i>
                            <p>Laporan</p>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('partner.profile.edit') }}" class="nav-link {{ request()->routeIs('partner.profile.*') ? 'active' : '' }}">
